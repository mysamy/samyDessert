...
*(Déplacé depuis dossier-projet.md §22)*

| Terme | Définition |
|-------|------------|
| **Atomic Design** | Méthode de conception d'interfaces qui découpe les composants en atomes (bouton), molécules (champ de formulaire), organismes (formulaire complet) et templates |
| **ARIA** | Accessible Rich Internet Applications — ensemble d'attributs HTML permettant aux lecteurs d'écran de comprendre les interfaces dynamiques |
| **AssetMapper** | Outil Symfony qui remplace Webpack Encore pour gérer les fichiers JavaScript et CSS, sans étape de bundling |
| **CSRF** | Cross-Site Request Forgery — attaque où un site malveillant soumet un formulaire à la place de l'utilisateur. Les tokens CSRF protègent contre ça |
| **Design system** | Ensemble de règles, composants et tokens (couleurs, typographie, espacements) qui garantissent la cohérence visuelle d'un projet |
| **Design token** | Variable CSS qui donne un nom sémantique à une valeur (ex : `--color-accent` pour la couleur framboise) |
| **Doctrine ORM** | Bibliothèque PHP qui fait le lien entre les objets PHP (entités) et les tables SQL |
| **Docker** | Outil de conteneurisation qui emballe une application et ses dépendances dans des conteneurs isolés |
| **EasyAdmin** | Bundle Symfony qui génère automatiquement une interface d'administration (CRUD) |
| **Entité** | Classe PHP représentant une table de base de données. Chaque propriété correspond à une colonne |
| **Fixture** | Données de test insérées en base pour le développement ou les tests |
| **JWT** | JSON Web Token — format de jeton signé utilisé pour l'authentification sans session |
| **Migration** | Fichier SQL versionné qui décrit une modification du schéma de base de données |
| **MVC** | Model-View-Controller — sépare les données (Model), la logique (Controller) et l'affichage (View/Twig) |
| **ORM** | Object-Relational Mapper — traduit les objets PHP en requêtes SQL et inversement |
| **PHPUnit** | Framework de tests automatisés pour PHP |
| **Repository** | Classe PHP chargée de récupérer des entités depuis la base de données |
| **Service** | Classe PHP qui contient de la logique métier réutilisable, injectable via l'injection de dépendances |
| **VichUploader** | Bundle Symfony qui gère l'upload de fichiers (images) et leur stockage |
| **WCAG** | Web Content Accessibility Guidelines — normes internationales d'accessibilité du web (niveaux A, AA, AAA) |
| **WebTestCase** | Classe Symfony pour les tests fonctionnels — simule un navigateur faisant de vraies requêtes HTTP |
